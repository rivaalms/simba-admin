...
const state : Ref <API.Request.Form.Supervisor> = ref({
...
const submit = async () => {
   ...
   try {
      ...
      store.notify('success', message, 'supervisor-form')
      if (store.dialog.callback) store.dialog.callback()
      store.clearDialog()
   } catch (e: unknown) {
      const error = e as API.Error
      store.notify('error', `${error.response?._data?.message || error}`, 'supervisor-error')
   } finally {
      loading.value = false
   }
}
